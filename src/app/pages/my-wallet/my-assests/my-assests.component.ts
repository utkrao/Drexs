...
@Component({
  selector: 'app-my-assests',
  templateUrl: './my-assests.component.html',
  styleUrls: ['./my-assests.component.scss']
})

export class MyAssestsComponent implements OnInit , OnDestroy{
  ...
  checkPrevious(){
    this.route.queryParams.subscribe(params => {
      if (params.action === 'send') {
        ...
        if(this.storage.get('swap-action-data')){
          const { type, coinAsset } = this.storage.get('swap-action-data');
          this.swap(type,coinAsset);
        }
      }
  ...
  swap(type: string, coinAsset: any) {
    this.storage.set('swap-action-data',{'type':type, 'coinAsset': coinAsset})
    this.common.toggleModal({ show: true, type: type,  coinAsset: coinAsset, action: 'swap', walletList: this.walletAssetList });
  }
